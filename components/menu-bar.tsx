...
import { Home, Cpu, Info, Phone, ShoppingCart } from "lucide-react"
    ...
    >
      <motion.div
        className={`absolute -inset-2 bg-gradient-radial from-transparent ${
          isDarkTheme
            ? "via-blue-400/30 via-30% via-purple-400/30 via-60% via-cyan-400/30 via-90%"
            : "via-blue-400/20 via-30% via-purple-400/20 via-60% via-cyan-400/20 via-90%"
        ...
        {menuItems.map((item) => {
          ...
          return (
            <motion.li key={item.label} className="relative">
              <motion.div
                className={`block rounded-xl overflow-visible group relative ${isActive ? "bg-background/50" : ""}`}
